...
def _download_model(model_id: str, model: Dict[str, Any], target_path: Path) -> None:
    ...
    if kind == "civitai":
        _download_civitai_model(model_id, source, target_path)
        return
    if kind == "huggingface":
...
def _download_civitai_model(
    model_id: str, source: Dict[str, Any], target_path: Path
) -> None:
    version_id = source["version_id"]
    download_url = _resolve_civitai_download_url(version_id)
...
def _resolve_civitai_download_url(version_id: int) -> str:
